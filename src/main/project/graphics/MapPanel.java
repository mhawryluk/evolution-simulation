...
public class MapPanel extends JLayeredPane implements ActionListener {
                ...
                if (animalSelected == null) {
                    ...
                    if (animal != null) {
                        ...
                        Object paneInput = JOptionPane.showInputDialog(null,
                                ...
                                new ImageIcon(pics.getPicture(animalSelected.getOrientation().getIndex())),
                                null, "10");
    ...
    public void paint(Graphics g) {
        ...
        for (int i = 0; i < map.dimensions.width; i++) {
            for (int j = 0; j < map.dimensions.height; j++) {
                ...
                if (grass != null) {
                    g2D.drawImage(pics.getPicture(8 + ((position.x + position.y) % 2)), position.x * squareSize, position.y * squareSize, null, this);
                }
                ...
                if (animalsAtPosition != null) {
                    for (Animal animal : animalsAtPosition) {
                        ...
                        g2D.drawImage(pics.getPicture(orientation), position.x * squareSize, position.y * squareSize, null, this);
                    }
                }
            }
        }
            ...
            if (isAnimalObserved) {
                if (currentObservationDay == animalObservationDays) {
                    ...
                    JOptionPane.showMessageDialog(this, observedAnimalInfo, "Observed animal information",
                            JOptionPane.INFORMATION_MESSAGE, new ImageIcon(pics.getPicture(animalSelected.getOrientation().getIndex())));
    ...
    public void setStatPanel(StatPanel panel){
        statPanel = panel;
    }
    public void resume(){
        timer.start();
